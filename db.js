...
async function getStoreSettings(defaults) {
  await ensureStoreSettingsTable();
  const p = getPool();
  const [rows] = await p.query(
    "SELECT store_name, store_tagline FROM store_settings WHERE id = 1 LIMIT 1"
  );
  if (!rows.length) {
    return {
  ...
  return {
    storeName: rows[0].store_name || defaults.storeName,
    storeTagline: rows[0].store_tagline || defaults.storeTagline,
  };
}
